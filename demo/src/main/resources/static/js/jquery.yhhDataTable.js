...
(function ($) {
	var Method = {
		/*插件默认入口*/
		'defaulted':function($container,pluginName,methodName){
			var opts = optHandle.get($container,pluginName);
			if (methodName == 'defaulted'){
			...
			if (typeof options == 'string') {
				if (Method[options]){
					if ((typeof(param) == "object" && Object.prototype.toString.call(param).toLowerCase() == "[object object]" && !param.length) || param == undefined){
	    				...
	    				r.push(Method.defaulted($(this),pluginName,options));
					} else {
				...
				r.push(Method.defaulted($(this),pluginName,'defaulted'));
			}
		});
		if (r.length == 1) r = r[0];
		return r;
    ...
    $.fn.yhhDataTable.defaults = {
    	...
    	'paginate':{
    		...
    		'displayLen':100,  /*每页显示条数*/
    		'currentPage':1, /*当前页码（初始页码）*/ 
    		'changeDisplayLen': false,  /*改变每页显示数目*/
    		'displayLenMenu':[100,200,300,500] /*改变每页显示数目时的可选值*/
    	},
    	'tbodyData':{
